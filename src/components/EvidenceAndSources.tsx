    ...
    <section id="evidence-sources" className="scroll-mt-20 border-b border-[#D8D4CB] bg-[#F4F1EA] text-[#151515] py-16">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 space-y-12">
              ...
              >
                <div className="space-y-2.5">
                  <div className="flex items-center justify-between">
                    <span className="px-2 py-0.5 rounded text-xs font-bold bg-[#F4F1EA] border border-[#D8D4CB] text-[#6842C2]">
                      [{idx + 1}] {src.badge}
                    </span>
                    <span className="text-xs font-semibold text-[#716F68]">{src.year}</span>
                  </div>

                  <h4 className="font-serif font-medium text-sm text-[#151515] leading-snug">
                    {src.title}
                  </h4>
                  <div className="text-xs text-[#716F68] font-sans italic">
                    {src.authors}
                  </div>

                  <div className="pt-2 border-t border-[#D8D4CB] space-y-1">
                    <span className="text-xs text-[#A46622] block font-bold">
                ...
                <div className="pt-2.5 border-t border-[#D8D4CB] flex items-center justify-between text-xs">
        ...
        <div className="space-y-4">
          ...
          <div className="overflow-x-auto rounded-xl border border-[#D8D4CB] bg-[#ECE8DF] shadow-sm">
            <table className="w-full text-left text-xs font-mono">
              <thead className="border-b border-[#D8D4CB] bg-[#E3DFD5] text-[#2A2926] text-xs">
                <tr>
                ...
                {PAPER_LANDSCAPE.map((paper, idx) => (
                  <tr key={idx} className="hover:bg-[#E7E3D9] transition-colors">
                    <td className="py-3.5 px-4">
                      ...
                      <div className="text-xs text-[#716F68] font-sans mt-0.5">{paper.authors}</div>
                    </td>
                    <td className="py-3.5 px-3">
                      <span className="rounded bg-[#F4F1EA] border border-[#D8D4CB] px-2 py-0.5 text-xs text-[#2A2926] font-semibold">
                    ...
                    <td className="py-3.5 px-4 text-right">
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="inline-flex items-center gap-1 font-mono text-xs text-[#6842C2] font-semibold hover:underline"
                      >
                        <span>arXiv</span>
        ...
        <div className="rounded-xl border border-[#D8D4CB] bg-[#ECE8DF] p-6 space-y-3 font-mono text-xs shadow-sm">
          <div className="font-mono text-xs font-bold text-[#151515] tracking-wider">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {OFFICIAL_SOURCES.map((src, i) => (
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="font-mono text-xs font-bold text-[#167C80]">{src.venue}</span>
                ...
                <p className="mt-1 text-xs text-[#716F68] font-sans line-clamp-2 leading-relaxed">
                  {src.description}
                </p>
              </a>
            ))}
